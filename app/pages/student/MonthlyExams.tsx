...
export default function MonthlyExams() {
  ...
  const handleStartExam = (examId: number) => {
    navigate(`/student/exam/${examId}?type=course`);
  };
  ...
  const totalExams = courseExams.length + monthlyExams.length;
    ...
    <DashboardLayout role="student">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Tab Switcher */}
          <div className="flex rounded-xl bg-white border border-gray-200 p-1 shadow-sm mb-6">
            <button
            ...
            >
              All Course Exams ({courseExams.length})
            </button>
            <button
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${
